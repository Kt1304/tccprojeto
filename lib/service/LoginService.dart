...
Future<void> login(BuildContext context, TextEditingController emailController, TextEditingController passwordController, Function(bool) setLoading) async {
  ...
  if (response.success && response.result != null) {
    ...
    final bool? isValidado = loggedInUser.get<bool>('Validado');

    if (isValidado == null || !isValidado) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Seu cadastro ainda não foi validado. Aguarde o prazo necessário.')),
      );
      return;
    }
