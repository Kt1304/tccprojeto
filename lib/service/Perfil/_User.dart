...
class CadastroService {
  bool verificaCPFValido(String cpf) {
  ...
  Future<void> inserirUsuario({
    ...
    if (!verificaCPFValido(cpf)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('CPF inválido! Por favor, verifique.')),
      );
      return;
    }
    ...
    if (cpfExiste || emailExiste) {
      String mensagemErro = '';
      if (cpfExiste) mensagemErro += 'O CPF já está cadastrado.\n';
      if (emailExiste) mensagemErro += 'O e-mail já está cadastrado.';
...
class UserRepository {
  ...
  Future<bool> updateUserInfo(
      String objectId, Map<String, String> userData) async {
    try {
...
Future<void> fetchUserData(
    String objectId, Function(ParseObject) onSuccess, Function onError) async {
  try {
    final query = QueryBuilder<ParseObject>(ParseObject('_User'))
      ..whereEqualTo('objectId', objectId);
